...
- In Ruby, outdent access modifiers (`private`, `protected`, `public`, `module_function`) to the same indentation level as the enclosing class/module declaration, and enforce with RuboCop `Layout/AccessModifierIndentation` using `EnforcedStyle: outdent`.
